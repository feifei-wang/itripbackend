...
//			@ApiParam(required = true, name = "password", value = "密码",defaultValue="111111")			
	...
	@ApiImplicitParam(paramType="header",required=true,name="token",value="用户认证凭据",defaultValue="PC-b7782e510d27f86dd1ba7c0c915c2351-12-20190428151002-ec67a2")
	...
	@ApiOperation(value = "用户再次注销",httpMethod = "GET",
			protocols = "HTTP",produces = "application/json",
			response = Dto.class,notes = "客户端需在header中发送token")
	@ApiImplicitParam(paramType = "header",required = true,name = "token",value = "用户认证凭据",defaultValue = "")
	@RequestMapping(value="/logout1",method = RequestMethod.GET,produces = "application/json",headers = "token")
	public Dto logout1(HttpServletRequest request){
		//验证token
		String token=request.getHeader("token");
		if(!tokenService.validate(request.getHeader("user-agent"),token)){
			return DtoUtil.returnFail("TOKEN无效",ErrorCode.AUTH_TOKEN_INVALID);
