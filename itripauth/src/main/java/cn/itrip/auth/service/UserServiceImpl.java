...
public class UserServiceImpl implements UserService {
	...
	public void registerbyphone(ItripUser user) throws Exception {
		//发送手机验证码
		int rc=MD5.getRandomCode();
		String[] msgbody={String.valueOf(rc),String.valueOf(2)};
		smsService.send1(user.getUserCode(),"1",msgbody);
		//创建用户
		itripUserMapper.insertItripUser(user);
		//将验证码缓存到redis缓存数据库
		redisAPI.set("activation:"+user.getUserCode(),2*60,String.valueOf(rc));
